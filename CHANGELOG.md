...
### Added
- `Verifier.new(secrets: [...])` accepts an Array of secrets so rotated and previous secrets can both validate signatures during a rotation window
- Module-level `WebhookSignature.verify` now also accepts `secrets:` for the same purpose
